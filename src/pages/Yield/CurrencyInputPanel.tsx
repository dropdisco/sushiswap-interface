...
const ButtonSelect = styled.button<{ selected: boolean }>`
  align-items: center;
  height: 2.2rem;
  font-size: 20px;
  font-weight: 500;
...
const InputPanel = styled.div<{ hideInput?: boolean }>`
  ...
  position: relative;
  border-radius: ${({ hideInput }) => (hideInput ? '8px' : '0.325rem')};
...
export default function CurrencyInputPanel({
  ...
  const handleApprove = useCallback(async () => {
    try {
      ...
      const txHash = 2 //await onApprove()
      if (!txHash) {
        ...
        <InputRow style={hideInput ? { padding: '0', borderRadius: '8px' } : {}} selected={disableCurrencySelect}>
          {!hideInput && (
            <>
              ...
              />
              {account && lpToken && showMaxButton && label !== 'To' && (
                <StyledBalanceMax onClick={onMax}>MAX</StyledBalanceMax>
              )}
            </>
          )}
          <ButtonSelect selected={false} onClick={() => console.log()}>
            <Aligner>
